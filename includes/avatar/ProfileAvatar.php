...
class ProfileAvatar {
	...
	function getAvatarImage() {
		global $wgAvatarKey, $wgDBname, $wgUploadDirectory;

		$cache = MediaWikiServices::getInstance()->getMainWANObjectCache();
		$key = $cache->makeKey( 'user', 'avatar', $this->user_id, $this->avatar_size );
		$data = $cache->get( $key );
		...
		if ( $data ) {
			...
			$files = glob( $wgUploadDirectory . '/avatars/' . ( $wgAvatarKey ?? $wgDBname ) . '_' . $this->user_id . '_' . $this->avatar_size . "*" );
