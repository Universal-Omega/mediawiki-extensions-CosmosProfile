...
class SpecialUploadProfileAvatar extends SpecialUpload {
	/** @var string|null */
	...
	public function __construct( $request = null ) {
		parent::__construct();
		$this->mName = 'UploadProfileAvatar';
	}
	...
	protected function loadRequest() {
		...
		parent::loadRequest();
		$this->mUpload = new UploadProfileAvatar();
	...
	public function execute( $params ) {
		...
		$out->addModuleStyles( [
			...
			'ext.cosmosprofile.special.updateprofile.css', // Need this module for .profile-update-title styles at least, probably more
			'ext.cosmosprofile.special.uploadavatar.css'
		] );

		// Add JS
		$out->addModules( 'ext.cosmosprofile.special.uploadavatar.js' );
	...
	private function showSuccess( $ext ) {
		...
		$output .= '<tr>
			...
			'</td>
			<td class="image-cell">
				<img src="' . $uploadPath . '/avatars/' . ( $wgAvatarKey ?? $wgDBname ) . '_' . $uid . '_l.' . $ext . '?ts=' . $ts . '" alt="" />
			</td>
		</tr>';
		$output .= '<tr>
			<td class="title-cell" valign="top">' .
			...
			'</td>
			<td class="image-cell">
				<img src="' . $uploadPath . '/avatars/' . ( $wgAvatarKey ?? $wgDBname ) . '_' . $uid . '_ml.' . $ext . '?ts=' . $ts . '" alt="" />
			</td>
		</tr>';
		$output .= '<tr>
			<td class="title-cell" valign="top">' .
			...
			'</td>
			<td class="image-cell">
				<img src="' . $uploadPath . '/avatars/' . ( $wgAvatarKey ?? $wgDBname ) . '_' . $uid . '_m.' . $ext . '?ts=' . $ts . '" alt="" />
			</td>
		</tr>';
		$output .= '<tr>
			<td class="title-cell" valign="top">' .
			...
			'</td>
			<td class="image-cell">
				<img src="' . $uploadPath . '/avatars/' . ( $wgAvatarKey ?? $wgDBname ) . '_' . $uid . '_s.' . $ext . '?ts=' . $ts . '" alt="" />
			</td>
		</tr>';
		$output .= '<tr>
			<td>
	...
	function getAvatar( $size ) {
		global $wgAvatarKey, $wgDBname, $wgUploadDirectory, $wgUploadBaseUrl, $wgUploadPath;

		$uploadPath = $wgUploadBaseUrl ? $wgUploadBaseUrl . $wgUploadPath : $wgUploadPath;

		$files = glob(
			$wgUploadDirectory . '/avatars/' . ( $wgAvatarKey ?? $wgDBname ) . '_' .
